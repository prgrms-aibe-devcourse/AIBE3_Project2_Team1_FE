...
export const milestoneApi = {
  ...
  async get(milestoneId: number) {
    const { data } = await axiosInstance.get<MilestoneResponseDto>(`/milestones/${milestoneId}`);
    return data;
  },
...
export const kanbanApi = {
  ...
  async list(milestoneId: number) {
    const { data } = await axiosInstance.get<KanbanCardResponse[]>(
      `/milestones/${milestoneId}/cards`
    );
    return data;
  },
  // [ADD] ETag 조건부 GET
  async listConditional(milestoneId: number, etag?: string) {
    return getWithEtag<KanbanCardResponse[]>(`/milestones/${milestoneId}/cards`, etag);
  },

  // POST /api/v1/milestones/{milestoneId}/cards  body: { title, columnId }
  async create(milestoneId: number, columnId: string, title: string) {
    const { data } = await axiosInstance.post<KanbanCardResponse>(
      `/milestones/${milestoneId}/cards`,
      {
        title,
        columnId,
  ...
  async update(milestoneId: number, cardId: number, patch: CardPatchRequest) {
    const { data } = await axiosInstance.patch<KanbanCardResponse>(
      `/milestones/${milestoneId}/cards/${cardId}`,
      patch
    );
    return data;
  },
  ...
  async remove(milestoneId: number, cardId: number) {
    await axiosInstance.delete<void>(`/milestones/${milestoneId}/cards/${cardId}`);
  },
};
...
export const calendarApi = {
  ...
  async update(milestoneId: number, eventId: number, patch: EventPatchRequest) {
    const { data } = await axiosInstance.patch<CalendarEventResponse>(
      `/milestones/${milestoneId}/events/${eventId}`,
      patch
    );
    return data;
  },
  ...
  async remove(milestoneId: number, eventId: number) {
    await axiosInstance.delete<void>(`/milestones/${milestoneId}/events/${eventId}`);
  },
};
...
export const filesApi = {
  ...
  async list(milestoneId: number) {
    const { data } = await axiosInstance.get<FileResponseDto[]>(`/milestones/${milestoneId}/files`);
    return data;
  },
  // [ADD] ETag 조건부 GET
  async listConditional(milestoneId: number, etag?: string) {
  ...
  async remove(milestoneId: number, fileId: number) {
    await axiosInstance.delete<void>(`/milestones/${milestoneId}/files/${fileId}`);
  },
...
export const teamApi = {
  ...
  async list(milestoneId: number) {
    const { data } = await axiosInstance.get<TeamMemberDto[]>(
      `/milestones/${milestoneId}/team-members`
    );
    return data;
  },
  ...
  async create(milestoneId: number, payload: Partial<TeamMemberDto>) {
    const { data } = await axiosInstance.post<TeamMemberDto>(
      `/milestones/${milestoneId}/team-members/one`,
      payload
    );
    return data;
  },
  ...
  async update(milestoneId: number, memberId: number, payload: Partial<TeamMemberDto>) {
    const { data } = await axiosInstance.patch<TeamMemberDto>(
      `/milestones/${milestoneId}/team-members/${memberId}`,
      payload
    );
    return data;
  },
  ...
  async remove(milestoneId: number, memberId: number) {
    await axiosInstance.delete<void>(`/milestones/${milestoneId}/team-members/${memberId}`);
  },
};
